...
public class ImpInterfazServidor extends UnicastRemoteObject implements InterfazServidor {
	...
	public synchronized void RegistroCallBack(InterfazCliente ObjetoCallBack) throws RemoteException {
		if (!(ListaClientes.contains(ObjetoCallBack))) {
			ListaClientes.addElement(ObjetoCallBack);
	...
	private synchronized void EfectuarCallBack() throws RemoteException {
		for (int i = 0; i < ListaClientes.size(); i++) {
			...
			Cliente.Senial("Eres el cliente n�mero: " + i);
		} // for
	}
}